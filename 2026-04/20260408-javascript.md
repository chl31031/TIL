...
* **`switch` (값의 매칭)** 
    * **특징**: 특정 변수의 **명확한 값**에 따라 실행문을 나눌 때 가독성이 좋습니다.(하나의 변수에 대해 여러 경우를 처리) `break`를 빼먹으면 다음 case까지 실행되니 주의해야 합니다.
    * **구조**:
        ```javascript
        switch (browser) {
            case 'Chrome':
...
---
### 💡 오늘의 인사이트
* Java(정적)와 JS(동적)의 타입 결정 시점 차이를 명확히 이해함.
* 현대적인 개발을 위해 ES6 문법과 엄격한 비교(`===`)의 습관화가 중요함을 느낌.